...
class UserController extends ApiController
{
    ...
    private function decryptSo($str, $key)
    {
        ...
        //decode the input value, it's base64 encoded by Delphi
        $enc = base64_decode($str);
        //make a hash of the Key, it'll be truncated when used
        $k = hash(SHA256, $key, true);

        //get the size of the IV
        $iv_size = mcrypt_get_iv_size(MCRYPT_3DES, MCRYPT_MODE_CBC);
        //fill a dummy IV with the same values that Delphi has (0,0,0,0,0,0,0,0)
        $empty_iv = str_repeat(chr(0), $iv_size);
        //encrypt an empty string with this empty IV to match the values generated by Delphi
        $iv = mcrypt_encrypt(MCRYPT_3DES, substr($k, 0, $keysize), "", MCRYPT_MODE_CBC, $empty_iv);
        //decrypt the value with the matching IV and Key
